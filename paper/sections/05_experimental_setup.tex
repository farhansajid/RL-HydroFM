...
\subsection{Multi-Sensor Spatial and Temporal Alignment}
Because Sentinel-2 MSI multi-spectral bands vary in ground sampling distance (10\,m for visible and NIR, 20\,m for red-edge and SWIR, and 60\,m for atmospheric bands), all multi-spectral bands are resampled to a uniform 10\,m grid using bicubic spline interpolation prior to foundation model encoding. For multi-modal Sen12-Flood tiles, Sentinel-1 C-band SAR Level-1 Ground Range Detected (GRD) products at 10\,m pixel spacing are coregistered to the Sentinel-2 optical grid using the European Space Agency SNAP toolbox with Copernicus 30\,m Global Digital Elevation Model (DEM) range-Doppler terrain correction. Radiometric calibration applies thermal noise removal and converts digital numbers to gamma-nought backscattering coefficients $\gamma^0 = \sigma^0 / \cos\theta_{\text{inc}}$, where $\theta_{\text{inc}}$ is the local radar incidence angle, eliminating topography-induced illumination biases across steep terrain.
